...
**[confluence-pull-docs](./workflows/confluence-pull-docs/)** - Fetch Confluence pages and save locally as markdown

Automated workflow for:

- Identify Confluence pages to fetch (by page IDs)
- Fetch pages using Confluence MCP server
- Convert to markdown format
- Save to local directory (docs/confluence-sync)
- Checkpoint after each page for recovery

**Key Features:**

- Per-page checkpointing for reliability
- Confluence MCP integration
- Markdown conversion for local editing
- Useful for syncing documentation

**[create-jira](./workflows/create-jira/)** - Create Jira issues (Bug/Task/Story) with intelligent routing

Automated workflow for:

- Interactive issue type selection (Bug, Task, or Story)
- Comprehensive details collection (observed/expected behavior, outcome, value)
- Platform and priority selection
- Device information capture for bugs
- Screenshot and log attachment tracking
- Gherkin acceptance criteria generation
- Automatic routing to backlog or current sprint based on urgency
- Label application and positioning control

**Key Features:**

- Unified interface for all issue types
- Intelligent routing (urgent → sprint, normal → backlog)
- Rich detail collection with context-specific prompts
- Native Jira checklist creation
- Supports immediate sprint start for critical issues
- Multi-platform support (iOS, Android, Web, Backend, Shared)

**[reject-issue](./workflows/reject-issue/)** - Reject story and move back to backlog with feedback

Automated workflow for:

- Issue key identification
- Rejection reason collection
- Move issue back to backlog
- Add structured rejection feedback comment
- Notify stakeholders of rejection

**Key Features:**

- Simple rejection workflow (4 steps)
- Structured feedback collection
- Automatic backlog transition
- Comment added to Jira for transparency
- Stakeholder notification support
